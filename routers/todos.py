...
async def edit_todo(request: Request, todo_id: int = Path(gt=0), db: Session = Depends(get_db)):
...
async def change_todo(request: Request, title: str = Form(...), description: str = Form(...),
                priority: int = Form(...), db: Session = Depends(get_db), todo_id: int = Path(gt=0)):
...
async def delete_todo(request: Request, db: Session = Depends(get_db), todo_id: int = Path(gt=0)):
